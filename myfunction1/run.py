...
def list_inactive_protectedhost():
    ...
    API_ENDPOINT = ALERT_LOGIC_API_TM_URL + CUST_ID + "/protectedhosts?config.collection_method=agent&status.status=offline&type=host&offset=0"
...
def list_inactive_source():
    ...
    API_ENDPOINT = ALERT_LOGIC_API_LM_URL + CUST_ID + "/sources/?type=syslog&status=offline&offset=0"
    REQUEST = requests.get(API_ENDPOINT, headers=HEADERS, auth=(API_KEY,''))
    RESULT = json.loads(REQUEST.text)
    return RESULT
